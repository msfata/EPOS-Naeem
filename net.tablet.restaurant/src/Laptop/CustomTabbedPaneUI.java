...
//	private int anchoFocoV = inclTab;
	...
	protected void paintTabBackground(Graphics g, int tabPlacement, int tabIndex, int x, int y, int w, int h,
		...
		case TOP:
		default:
			xp = new int[] { x, x, x + 3, x + w - inclTab - 6, x + w - inclTab - 2, x + w - inclTab, x + w - inclTab,
					x };
		...
		if (isSelected) {
			...
			if (tabPane.isEnabled() && tabPane.isEnabledAt(tabIndex)) {
				...
				GradientPaint gradientShadowTmp = new GradientPaint(0, 0, new Color(255, 255, 200), 0, y + h / 2,
	 ...
	 * { g.setFont(font); View v = getTextViewForTab(tabIndex); if (v != null) { //
	 * html v.paint(g, textRect); } else { // plain text int mnemIndex =
	 * tabPane.getDisplayedMnemonicIndexAt(tabIndex);
	 * 
	 * if (tabPane.isEnabled() && tabPane.isEnabledAt(tabIndex)) { Color fg =
	 * tabPane.getForegroundAt(tabIndex); if (isSelected && (fg instanceof
	 * UIResource)) { Color selectedFG =
	 * UIManager.getColor("TabbedPane.selectedForeground"); if (selectedFG != null)
	 * { fg = selectedFG; } } g.setColor(fg);
	 * SwingUtilities2.drawStringUnderlineCharAt(tabPane, g, title, mnemIndex,
	 * textRect.x, textRect.y + metrics.getAscent());
	 * 
	 * } else { // tab disabled //PAY ATTENTION TO HERE
